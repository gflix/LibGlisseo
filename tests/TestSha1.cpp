...
TEST(Sha1, EmptyMessageHash)
{
    ...
    sha1.reset();
    EXPECT_EQ(Glisseo::Conversion::binToHex(sha1.getHash()), expected);
}
...
TEST(Sha1, TestMessageHash)
{
    ...
    EXPECT_EQ(Glisseo::Conversion::binToHex(sha1.getHash()), expectedA);
    sha1.appendMessage(messageB);
    EXPECT_EQ(Glisseo::Conversion::binToHex(sha1.getHash()), expectedB);
